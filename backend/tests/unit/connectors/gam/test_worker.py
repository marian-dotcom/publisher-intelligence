...
class Service:
    ...
    async def run_extract(self, **kwargs: Any) -> NormalizedExtract:
        ...
        if self.error:
            raise self.error
        return NormalizedExtract("Europe/Bucharest", (), {}, ())
...
def valid_payload() -> dict[str, Any]:
    return {
        ...
        "scheduled_run_key": "gam-unit-run",
    }


async def test_worker_accepts_only_tenant_owned_payload_without_token() -> None:
    queue = Queue()
    service = Service()
    await handle_job(cast(Any, queue), lease(valid_payload()), 5, None, None, cast(Any, service))
    assert queue.completed is True
    assert len(service.calls) == 1
    assert "token" not in str(service.calls[0]).lower()


async def test_worker_retries_quota_with_bounded_backoff() -> None:
    queue = Queue()
    service = Service(ConnectorError("QUOTA_LIMIT", retryable=True, message="quota"))
    await handle_job(
        cast(Any, queue), lease(valid_payload(), attempt=3), 5, None, None, cast(Any, service)
    )
...
async def test_worker_rejects_extra_credential_field() -> None:
    queue = Queue()
    service = Service()
    payload = valid_payload()
    payload["access_token"] = "forbidden"
    await handle_job(cast(Any, queue), lease(payload), 5, None, None, cast(Any, service))
    assert queue.failure is not None
